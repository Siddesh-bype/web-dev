...
app.get("/filter", (req, res) =>{
  ...
  const found= jokes.filter((joke)=>joke.type==type);
  res.json(found); 
});
//4. POST a new joke
...
app.put("/jokes/:id", (req,res)=>{
  ...
  const search=jokes.findIndex((joke) => joke.id === id);
    jokes[search]=replacementjoke;
    res.json(replacementjoke);
});
//6. PATCH a joke
app.patch("/jokes/:id", (req,res)=>{
  const id =parseint(req.params.id);
    const exist=jokes.find((joke) => joke.id === id);
  const replacementjoke={
      id :id,
